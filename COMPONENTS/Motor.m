classdef Motor < Component
    ...
    properties
        L {mustBeParam} = symParam('L',1.17e-4) % Leakage Inductance - H
        J {mustBeParam} = symParam('J',6.5e-6) % Mechanical rotational inertia - Modified to better reflect Ferry's simulation results
        K_t {mustBeParam} = symParam('K_t',0.00255) % Torque/Speed Coefficient - Nm/A
        R_1 {mustBeParam} = symParam('R_1',0.117) % Phase Resistance - Ohms
        B_v {mustBeParam} = symParam('B_v',2.415e-6) % Viscous Friction - N*m*s
        T_c {mustBeParam} = symParam('T_c',0) % Coulomb Friction
